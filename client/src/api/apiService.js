...
apiService.register = (name, email, password, role) =>
  apiService.post("/auth/register", { name, email, password, role });
...
apiService.resetPassword = (token, password) =>
  apiService.patch(`/auth/reset-password/${token}`, { password });
apiService.verifyEmail = (verificationToken) =>
  apiService.get(`/auth/verify-email/${verificationToken}`);

apiService.requestBookingCancellation = (bookingId) =>
  apiService.put(`/bookings/${bookingId}/request-cancellation`);
apiService.requestBookingCompletion = (bookingId) =>
  apiService.put(`/bookings/${bookingId}/request-completion`);
apiService.getCancellationRequests = () =>
  apiService.get("/bookings/owner/cancellation-requests");
apiService.respondToCancellation = (bookingId, decision) =>
  apiService.put(`/bookings/${bookingId}/respond-cancellation`, { decision });
apiService.getCompletionRequests = () =>
  apiService.get("/bookings/owner/completion-requests");
apiService.respondToCompletion = (bookingId, decision) =>
  apiService.put(`/bookings/${bookingId}/respond-completion`, { decision });

apiService.createParkingArea = (data) => {
  const formData = new FormData();
  Object.keys(data).forEach((key) => formData.append(key, data[key]));
  return apiService.post("/parking-areas", formData, {
...
apiService.updateParkingArea = (id, data) => {
  const formData = new FormData();
  Object.keys(data).forEach((key) => formData.append(key, data[key]));
  return apiService.put(`/parking-areas/${id}`, formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
};
apiService.deleteParkingArea = (id) =>
...
apiService.bookSlot = (bookingData) =>
  apiService.post("/user/book-slot", bookingData);
apiService.getUserBookings = () => apiService.get("/user/bookings");
apiService.getUserBookingHistory = () =>
  apiService.get("/user/bookings/history");
...
apiService.updateUserPassword = (passwordData) =>
  apiService.put("/profile/update-my-password", passwordData);
apiService.uploadProfilePicture = (file) => {
  const formData = new FormData();
  formData.append("profilePic", file);
  return apiService.post("/profile/upload-picture", formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
};
