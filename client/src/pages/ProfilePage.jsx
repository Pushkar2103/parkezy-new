    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (response.user) {
                ...
                setImageFeedback({ message: response.message || 'Upload failed.', type: 'error' });
            }
        } catch (error) {
            setImageFeedback({ message: 'A server error occurred.', type: 'error' });
        } finally {
            setUploading(false);
        }
    };
    ...
    const handleProfileSubmit = async (e) => {
        ...
        try {
            ...
            if (response.email) {
                ...
                setProfileFeedback({ message: response.message || 'Failed to update profile.', type: 'error' });
            }
        } catch (error) {
            setProfileFeedback({ message: 'A server error occurred.', type: 'error' });
        }
    };
    ...
    const handlePasswordSubmit = async (e) => {
        ...
        try {
            ...
            if (response.message === 'Password updated successfully') {
                setPasswordFeedback({ message: response.message, type: 'success' });
                setPasswordData({ currentPassword: '', newPassword: '' });
            } else {
                setPasswordFeedback({ message: response.message, type: 'error' });
            }
        } catch (error) {
            setPasswordFeedback({ message: 'A server error occurred.', type: 'error' });
        }
    };
                ...
                <div className="md:col-span-2 space-y-8">
                    <div className="bg-white p-8 rounded-xl shadow-lg">
                        ...
                        <form onSubmit={handleProfileSubmit} className="space-y-4">
                            ...
                            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg cursor-pointer">Save Profile Changes</button>
                        </form>
                    </div>
                    <div className="bg-white p-8 rounded-xl shadow-lg">
                        ...
                        <form onSubmit={handlePasswordSubmit} className="space-y-4">
                            ...
                            <button type="submit" className="w-full bg-gray-700 hover:bg-gray-800 text-white font-bold py-2 px-4 rounded-lg cursor-pointer">Update Password</button>
                        </form>
