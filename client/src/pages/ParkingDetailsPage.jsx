    ...
    const handleBookingConfirm = async (e) => {
        ...
        try {
            ...
            if (response.booking) {
                ...
                setError(response.message || 'Failed to book the slot.');
            }
        } catch (err) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-8 rounded-xl shadow-2xl max-w-lg w-full relative">
                ...
                <form onSubmit={handleBookingConfirm} className="space-y-4">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition duration-300 disabled:bg-blue-400">
...
const ParkingDetailsPage = () => {
  ...
  const handleBookingSuccess = (booking) => {
    ...
    setBookingSuccessMessage(`Your booking for slot ${booking.parkingSlot?.slotNumber || 'selected'} is confirmed!`);
    fetchDetails();
  };

  if (loading) return <div className="text-center py-10">Loading Parking Details...</div>;
